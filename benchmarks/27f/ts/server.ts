...
function authorize(call: any, callback: any) {
  ...
  callback(null, {
    allowed,
    decisionId: `decision-${req.correlationId || "local"}`,
    reason: allowed ? "allowed" : "missing_context",
  });
}
...
function createOrder(call: any, callback: any) {
  ...
  for (const item of req.items ?? []) {
    subtotal += Number(item.quantity || 0) * Number(item.unitPrice?.amountMinor || 0);
  }
  const tax = Math.floor(subtotal * 0.05);
  callback(null, {
    orderId: req.orderId,
    status: "NEW",
    subtotal: money(subtotal, req.currency || "INR"),
    tax: money(tax, req.currency || "INR"),
    total: money(subtotal + tax, req.currency || "INR"),
  });
}

function getOrderStatus(call: any, callback: any) {
  callback(null, {
...
function getOrderDetails(call: any, callback: any) {
  callback(null, {
    orderId: call.request.orderId,
    status: "PREPARING",
    ...
    tax: money(50),
    total: money(1050),
    attributes: { source: "benchmark" },
  });
}

function getMenuItem(call: any, callback: any) {
  callback(null, {
    itemId: call.request.itemId,
    name: "Benchmark Item",
    description: "Deterministic benchmark menu item",
    basePrice: money(250),
    available: true,
    tags: ["benchmark", "vegetarian"],
  });
}
...
function getMenu(call: any, callback: any) {
  ...
  const items = Array.from({ length: count }, (_, i) => ({
    ...
    description: "Deterministic menu payload for RPC benchmarking",
    price: money(250 + (i % 20)),
    available: i % 17 !== 0,
    tags: ["benchmark", "menu"],
  }));
  callback(null, { items });
...
server.bindAsync(`0.0.0.0:${port}`, grpc.ServerCredentials.createInsecure(), (err) => {
  if (err) throw err;
  server.start();
  console.log(`dawat benchmark TS server listening on ${port}`);
});
